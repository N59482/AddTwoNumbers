    ...
    ListNode* addTwoNumbers(ListNode* l1, ListNode* l2)
    {
        ...
        } else if (l1 != nullptr && l2 == nullptr) // остался только второй список
        {
            ...
            while(l1 != nullptr)
            {
                ...
                if (l1->val > 9)
                    {
                        cout<<"Значение списка "<<l1->val<<"больше 9"<<endl;
                        if(l1->next != nullptr)
                           {
                            cout<<"В списке следующий элемент существует и равен "<<l1->next->val<<" +1 = ";
                            l1->next->val++;
                            cout<<l1->next->val<<endl;
                            l1->val = l1->val % 10;
                            cout<<"Сооттветственно запихиваем элемент = "<<l1->val<<endl;
                           } 
                        else {overnine = 1; cout<<"Следующего элемента нет овернайн = 1"<<endl; l1->val = l1->val % 10;
                            cout<<"Сооттветственно запихиваем элемент = "<<l1->val<<endl;}
                    }
        ...
        if (l1 == nullptr && l2 == nullptr && overnine) // списки пусты но остался overnine
        {
